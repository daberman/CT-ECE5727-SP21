...
// ADC/DAC sampling rate in Hz - this corresponds to 2 channel sampling, so audio sample rate is 0.5x
//#define AUDIO_SAMPLING_RATE		1000
#define AUDIO_SAMPLING_RATE	  96000 // 48kHz audio

// Number of samples to record/playback
#define NR_AUDIO_SAMPLES		(NR_SEC_TO_REC_PLAY*AUDIO_SAMPLING_RATE)
//#define NR_AUDIO_SAMPLES		16
...
static XAxis_Switch sStreamSwitch;
...
int main(void)
{
	...
	Status = fnInitAudio(&sStreamSwitch);
	if(Status != XST_SUCCESS) {
	...
	xil_printf("----------------------------------------------------------\r\n");
	xil_printf("  Controls:\r\n");
//	xil_printf("  BTN0: Play TEST TONE\r\n");
	xil_printf("  BTN1: Record from MIC\r\n");
	xil_printf("  BTN2: Playback Recording\r\n");
	xil_printf("  BTN3: Stream from MIC\r\n");
...
//	xil_printf("  BTN3: Record from LINE IN\r\n");
    ...
    while(1) {
		...
		if (Demo.fDmaS2MMEvent)
		{
			...
			Xil_Out32(PMOD_CONTROL_REG, 0x00000000);
...
//						xil_printf("\r\nStart Tone...\r\n");
//						fnSetHpOutput();
//						fnAudioPlay(sAxiDma,sStreamSwitch,NR_AUDIO_SAMPLES,1);
//						Demo.fAudioPlayback = 1;
				...
				case 'l':
					if (!Demo.fAudioRecord && !Demo.fAudioPlayback)
					{
						xil_printf("\r\nStart Playback...");
						fnSetHpOutput();
						fnAudioPlay(sAxiDma,sStreamSwitch,NR_AUDIO_SAMPLES,0);
						Demo.fAudioPlayback = 1;
					}
					else
					{
